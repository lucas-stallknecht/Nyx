...
void main()
{
    ...
    vec4 clip_origin = vec4(uv * 2.0 - 1.0 /* ndc */ , depth, 1.0);
    ...
    vec3 ss_origin = vec3(clip_origin.xy * 0.5 + 0.5, clip_origin.z);
    vec3 ss_end = vec3(ndc_end.xy * 0.5 + 0.5, ndc_end.z);

    ivec2 origin_px = ivec2(ss_origin.xy * size);
    ivec2 end_px = ivec2(ss_end.xy * size);
    ...
    // DO NOT normalize as screen space is not linear
    vec3 ss_step = (ss_end - ss_origin) / float(max_dist);

    vec3 pos = ss_origin + ss_step;
    bool hit = false;
